...
def non_max_suppression(prediction, conf_thres=0.7, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    #print("prediction[..., 4]",prediction[..., 4].shape)
    #print("xc",xc.shape)
...
def kmean_anchors(path='./data/coco128.yaml', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    ...
    for _ in pbar:
        ...
        while (v == 1).all():  # mutate until a change occurs (prevent duplicates)
            v = ((npr.random(sh) < mp) * npr.random() * npr.randn(*sh) * s + 1).clip(0.7, 7.0)
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            for j, box in enumerate(boxes.T):
                ...
                if gt or conf[j] > 0.7:  # 0.3 conf thresh
