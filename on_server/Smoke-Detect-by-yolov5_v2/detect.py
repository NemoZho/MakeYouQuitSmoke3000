...
def sendline(msg, img):
    LINE_ACCESS_TOKEN="DAn25MzC809NjVVi3CaKvkGkAihN10qFavUglKRV14T"
...
def on_message(client, obj, msg):
    global smoke_deteceted
    if msg.payload.decode("utf-8")=="True":
        smoke_deteceted +=1
    elif msg.payload.decode("utf-8")=="False":
        if smoke_deteceted > 0:
            smoke_deteceted -= 0.5
    #print("smoke")
    #print(smoke_deteceted)
...
def listen_smoke():
    ...
    client_s.connect(host="192.168.87.2", port=1883)
    client_s.subscribe('smoke', 0)
...
def detect(save_img=False):
    ...
    client_p.connect(host="192.168.87.2", port=1884)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                for c in det[:, -1].unique():
                    ...
                    if n>=1:
                        enable += 1
                        if enable > 75 and smoke_deteceted>7:
                            print("detector sees someone smoking :( ")
                            client_p.publish(topic="violation", payload=True)
                            msg = "有人在抽菸!  "
                            cv2.imwrite("detect.jpg",im0)
                            send_img = "detect.jpg"
                            sendline(msg, send_img)
                            enable=0
                            smoke_deteceted=0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=r'runs\evolution\weights\test.pt', help='model.pt path(s)')
    ...
    #print(opt)
    t=threading.Thread(target=listen_smoke)
    t.start()
    with torch.no_grad():
